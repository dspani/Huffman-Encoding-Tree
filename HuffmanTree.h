...
private:
	...
	void rebuildTree(BinaryNode * node, string element, string route);
	void rebuildTree(ifstream & file);
	void getCodes(BinaryNode * node, string &code, char letter);
	...
	bool contains(BinaryNode * node, char letter);

	string decode(BinaryNode * t, char bit, ifstream &binary);
	string decode(vector<string> bytes, BinaryNode * node);
	//void decodeRecursive(BinaryNode * node, vector<string> &bytes, string &letter);
...
public:	
	...
	vector<string> encode(string stringToEncode);
	string decode(vector<string> endcodedBytes);	
